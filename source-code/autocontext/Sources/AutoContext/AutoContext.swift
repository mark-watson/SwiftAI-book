// AutoContext.swift
// Core AutoContext class: loads documents, builds BM25 + vector
// indices, and generates context-enriched prompts for an LLM.

import Foundation

// MARK: - Text Processing

// Common English stop words — these terms appear in almost every
// document and carry no discriminating power for BM25 retrieval.
private let stopWords: Set<String> = [
    "a", "an", "the", "and", "or", "but", "in", "on", "at",
    "to", "for", "of", "with", "by", "from", "is", "are",
    "was", "were", "be", "been", "being", "have", "has", "had",
    "do", "does", "did", "will", "would", "could", "should",
    "may", "might", "that", "this", "these", "those", "it",
    "its", "i", "you", "he", "she", "we", "they", "who",
    "which", "what", "how", "when", "where", "why", "not",
    "no", "so", "as", "if", "than", "then", "can", "also",
    "about", "up", "out", "there", "their", "them", "any",
    "all", "more", "other", "into", "such", "very", "just"
]

/// Lowercases `text`, strips punctuation, splits on whitespace,
/// and removes stop words — producing clean tokens for BM25.
    ...
    return text.lowercased()
        ...
        .compactMap { word -> String? in
            ...
            guard !cleaned.isEmpty,
                  !stopWords.contains(cleaned) else {
                return nil
            }
            return cleaned
        }
...
/// Splits `text` into individual sentences using heuristics.
func splitIntoSentences(_ text: String) -> [String] {
    ...
    while i < text.endIndex {
        ...
        let nextOk = atEnd || followedBy.contains(text[next])
        if terminators.contains(ch) && nextOk {
            let sentence = text[start...i]
                .trimmingCharacters(in: .whitespacesAndNewlines)
            if !sentence.isEmpty { sentences.append(sentence) }
            start = !atEnd ? next : text.endIndex
        }
        i = next
    }
    // Capture any trailing text without terminal punctuation.
    let remainder =
        text[start...].trimmingCharacters(in: .whitespacesAndNewlines)
    if !remainder.isEmpty { sentences.append(remainder) }
    return sentences.filter { !$0.isEmpty }
}

/// Groups sentences from `text` into chunks of `chunkSize` each.
func chunkText(_ text: String, chunkSize: Int = 3) -> [String] {
    let sentences = splitIntoSentences(text)
    var chunks: [String] = []
    var idx = 0
    while idx < sentences.count {
        let group = sentences[idx..<min(
            idx + chunkSize, sentences.count)]
...
/// Reads every `.txt` file in `directoryPath` and returns all chunks.
func loadAndChunkDocuments(directoryPath: String) -> [String] {
    ...
    guard
        ...
        fputs("[Warning] Could not open: \(directoryPath)\n", stderr)
        return []
    }
    var chunks: [String] = []
...
/// Hybrid RAG retriever that combines BM25 keyword search with
/// semantic vector search to build a context-enriched LLM prompt.
class AutoContext {
    let chunks: [String]
    let bm25: BM25Index
    let chunkEmbeddings: [[Double]]  // parallel to chunks

    /// Initialises AutoContext (embeddings require network calls).
    init(
        chunks: [String],
        bm25: BM25Index,
        chunkEmbeddings: [[Double]]
    ) {
        self.chunks = chunks
        self.bm25 = bm25
    ...
    /// Factory: load documents from `directoryPath`, build indices.
    static func build(
        ...
        guard !chunks.isEmpty else {
            fputs("[Error] No chunks in \(directoryPath)\n", stderr)
            return nil
        }
    ...
    /// Retrieves the most relevant chunks for `query` using hybrid
    /// search and formats them into a one-shot LLM prompt.
    ///
    /// - Parameters:
    ///   - query: The user's question.
    ///   - numResults: Maximum chunks per retriever.
    ///   - minBM25Score: BM25 threshold (default 1.75).
    ///   - minSimilarity: Cosine similarity threshold (default 0.75).
    func getPrompt(
        query: String,
        numResults: Int = 2,
        ...
        let bm25Docs = bm25.topN(
            numResults,
            for: queryTokens,
            minScore: minBM25Score)
        let bm25Results = bm25Docs.map { $0.joined(separator: " ") }
        print("BM25 found \(bm25Results.count) keyword results.")

        // 2. Dense search (cosine similarity on embeddings)
        let apiKey =
            ProcessInfo.processInfo.environment["GOOGLE_API_KEY"] ?? ""
        var vectorResults: [String] = []
        if let queryVec = await generateEmbedding(
            for: query, apiKey: apiKey) {
            let similarities = chunkEmbeddings
                .enumerated()
                .map { (i, emb) in
        ...
        print("Vector search found \(vectorResults.count) results.")
        ...
        print("Combined: \(uniqueResults.count) context chunks.")
