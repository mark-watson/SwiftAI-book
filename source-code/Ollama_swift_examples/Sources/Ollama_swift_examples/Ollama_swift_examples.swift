...
public actor OllamaService {
    ...
    public func chatStream(
        messages: [Ollama.Chat.Message],
        tools: [any Ollama.ToolProtocol] = []
    ) -> AsyncThrowingStream<Ollama.Client.ChatResponse, any Error> {
        AsyncThrowingStream { continuation in
            Task {
                do {
                    for try await chunk in try await client.chatStream(
                        model: model,
                        messages: messages,
                        tools: tools) {
...
extension Ollama.Tool {
    ...
    /// - Parameter arguments: The arguments dictionary from a call.
