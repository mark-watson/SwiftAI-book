...
struct OpenAI {
    ...
    private static func makeRequest<T: Encodable>(
        endpoint: String,
        body: T
    ) -> String {
        var responseString = ""
    ...
    static func chat(
        ...
        let response = makeRequest(
            endpoint: "/chat/completions", body: chatRequest)
        guard
    ...
    static func embeddings(text: String) -> [Float] {
        ...
        let response = makeRequest(
            endpoint: "/embeddings", body: embeddingRequest)
        guard
            let data = response.data(using: .utf8),
            let json = try? JSONSerialization.jsonObject(
                with: data) as? [String: Any],
            ...
            let embedding =
                dataArray.first?["embedding"] as? [NSNumber]
        else {
            return [1.23]
        }
